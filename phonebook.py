...
COLUMNS = (f"{'Фамилия':^21}|{'Имя':^17}|{'Отчество':^23}|"
           f"{'Организация':^23}|{'Рабочий телефон':^17}|"
           f"{'Телефон сотовый':^17}|\n")
TABLE_TOP = [COLUMN_SEPARATORS, COLUMNS, BORDERS, COLUMN_SEPARATORS]
INPUT_MESSAGE_1 = 'Пожалуйста, введите номер действия: '
INPUT_MESSAGE_2 = 'Введите номер записи, которую хотите изменить. ' \
                  'Введите 0, чтобы вернуться обратно '
INPUT_MESSAGE_3 = 'Номер параметра: '
INPUT_MESSAGE_4 = 'Введите номер записи, которую хотите удалить. ' \
                  'Введите 0, чтобы вернуться обратно '
...
class PhoneBook:
    ...
    def __init__(self, filename):
        ...
        self.finded_notes = []
        self.key_word = None
        self.columns_for_printing = ['Номер', 'Фамилия', 'Имя',
                                     'Отчество', 'Организация',
                                     'Телефон рабочий', 'Телефон сотовый']

    def read_file_and_choose_actions(self):
        """Функция считывает файл phonebook.txt и сохраняет данные
        в атрибуте self.all_filestrings, игнорируя шапку таблицы и
        пробельные строки. После предлагает выбор действия пользователю.
    ...
    def choose_the_action_with_empty_book(self):
        ...
        while True:
            ...
            action_number = self.check_user_action_input(
                    message=INPUT_MESSAGE_1)
            if action_number == 1:
                self.add_new_note()
                self.choose_the_action_with_phonebook()
                break
    ...
    def find_notes_by_one_param(self):
        """Функция в начале производит сброс параметров.
        Далее пользователь выбриает критерий, по которому
        хочет найти записи. Запускается поиск информации с
        проверками вводимых данных"""
        self.reset_params()
        ...
        if 1 <= action_number <= 3:
            self.key_word = self.input_text_info_and_check(
                    column_name='выбранный параметр', length=23)
            self.prepare_finded_lines_and_choose_action()
        elif action_number == 4:
            self.key_word = self.input_company_info_and_check(
                    column_name='выбранный параметр', length=23)
            self.prepare_finded_lines_and_choose_action()
        elif 5 <= action_number <= 6:
            self.key_word = self.input_phone_numbers_and_check(
                    column_name='номер телефона', length=17)
            self.prepare_finded_lines_and_choose_action()
    ...
    def choose_the_action_with_finded_notes(self):
        """Выбор действий после успешного поиска записей.
        Найденные записи сохраняются в атрибуте self.finded_notes"""
        while True:
    ...
    def choose_the_action_after_deleting(self):
        """Перезаписать файл phonebook.txt или внести оставшиеся записи
        из self.finded_notes в очередь на удаление"""
    ...
    def change_note(self):
        ...
        while True:
            ...
            if note_number == 0:
                ...
                self.all_filestrings.remove(self.finded_notes[note_number - 1])
                self.all_filestrings.append(
                    f'{self.surname:^21}|{self.name:^17}|'
                    f'{self.patronymic:^23}|{self.workplace:^23}|'
                    f'{self.work_phone_number[0]:^17}|'
                    f'{self.personal_phone_number:^17}|\n')
                self.finded_notes[note_number - 1] = (
                    f'{self.surname:^21}|{self.name:^17}|'
                    f'{self.patronymic:^23}|{self.workplace:^23}|'
                    f'{self.work_phone_number[0]:^17}|'
                    f'{self.personal_phone_number:^17}|\n')
    ...
    def prepare_query_list(self, query):
        """Подготовка списка query, который содержит
    ...
    def add_info_in_object(self):
        ...
        self.surname = self.input_text_info_and_check(
                column_name='фамилию', length=19)
        self.name = self.input_text_info_and_check(
                column_name='имя', length=17)
        self.patronymic = self.input_text_info_and_check(
                column_name='отчество', length=21)
        self.workplace = self.input_company_info_and_check(
                column_name='компанию работодателя', length=21)
        self.work_phone_number = self.input_phone_numbers_and_check(
                column_name='рабочий телефон', length=15),
        self.personal_phone_number = self.input_phone_numbers_and_check(
                column_name='мобильный телефон', length=17)
    ...
    def add_new_note(self):
        ...
        print('Добавьте поочередно данные согласно их типу')
        self.add_info_in_object()
        new_note = (f'{self.surname:^21}|{self.name:^17}|'
                    f'{self.patronymic:^23}|{self.workplace:^23}|'
                    f'{self.work_phone_number[0]:^17}|'
                    f'{self.personal_phone_number:^17}|\n')
    ...
    def input_text_info_and_check(self, column_name, length):
        ...
        while True:
            ...
            if column.isalpha() and len(column) <= length:
                ...
                print(f'Введите конкретно {column_name}. Без пробелов и цифр. '
                      f'Длина меньше или равно {length} знаков.')

    def input_company_info_and_check(self, column_name, length):
    ...
    def input_phone_numbers_and_check(self, column_name, length):
        ...
        while True:
            column = input(f'Введите {column_name} '
                           f'пользователя без пробелов: ')
            if column.isdigit() and len(column) <= length:
                return column
            else:
                print(f'Введите конкретно {column_name}. Без букв. '
                      f'Длина меньше или равно {length} знаков.')

    def sort_notes_and_rewrite_file(self):
        """Сортировка записей и запись в файл"""
        self.all_filestrings = sorted(self.all_filestrings,
